...
class Star(pygame.sprite.Sprite):
    ...
    def __init__(self, screen):
        ...
        # Load the star image and set its rect. If loading the image fails
        # (file not found or other pygame error), create a simple fallback
        # surface so the game can run without the external image.
        try:
            self.image = pygame.image.load("images/star.bmp").convert_alpha()
        except (pygame.error, FileNotFoundError):
            # Create a small circular star on a transparent surface
